...
import argparse
from HE_server_clear_nodp import HE_Computations
...
class AIM(Mechanism):
    ...
    def run(self, data, workload, num_synth_rows=None, initial_cliques=None):
        # rounds = self.rounds or 16 * len(data.domain) + 100
        ...
        for t in range(1, rounds + 1):
            ...
            print("Error in this round", np.sum((w - z)**2))
...
if __name__ == "__main__":
    ...
    args.save = f"../data/logs/run_{args.seed}/{dataset_name}_nodp/synth_aim_l2_eps_{args.epsilon}.csv" # data
    model_save_path = f"../data/logs/run_{args.seed}/{dataset_name}_nodp/aim_l2_generator_eps_{args.epsilon}.joblib" # model
    log_dir = f"../data/logs/run_{args.seed}/{dataset_name}_nodp/aim_l2_eps_{args.epsilon}.log"
